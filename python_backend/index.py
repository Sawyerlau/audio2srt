...
def extract_audio_from_video(video_path, log_callback=None):
    """从视频中提取音频为mp3，输出文件与视频同名"""
    def log(msg):
        if log_callback:
            log_callback(msg)
        else:
    ...
    log(f"正在从视频提取音频: {os.path.basename(video_path)}")
    
    # 检查文件是否存在
    if not os.path.exists(video_path):
        log(f"❌ 文件不存在: {video_path}")
        return None
    
    # 尝试使用 moviepy (兼容 1.x 和 2.x)
    clip = None
    try:
        try:
            # moviepy 2.x
            ...
            try:
                ...
                raise ImportError("moviepy 未安装")
        
        # 检查音频轨道
        if clip is None or clip.audio is None:
            log(f"⚠️ 该视频没有音频轨道，无法提取: {os.path.basename(video_path)}")
            return None
        
        clip.audio.write_audiofile(audio_path, bitrate='320k')
        
        if clip:
            clip.close()
        log(f"音频提取成功: {base_name}.mp3")
        return audio_path
        ...
        try:
            ...
            video.export(audio_path, format='mp3', bitrate='320k')
            log(f"音频提取成功: {base_name}.mp3")
            return audio_path
        except ImportError:
            log("pydub 也未安装。")
            log("请先安装: pip install moviepy 或 pip install pydub")
            log("注意：pydub 也需要系统安装 ffmpeg")
            return None
        except Exception as e:
            log(f"pydub 处理失败: {e}")
            return None
    except Exception as e:
        log(f"❌ 提取音频时出错: {type(e).__name__}: {e}")
        return None
...
def process_single_audio(audio_input, file_index, log_cb=None, is_local_file=False, filename_prefix=None):
    ...
    log(f"\n===== 开始处理第 {file_index} 个文件 =====")
    
    # 处理视频文件
    audio_to_process = audio_input
    if is_local_file and is_video_file(audio_input):
        audio_path = extract_audio_from_video(audio_input, log_cb)
        if not audio_path:
            log(f"第 {file_index} 个视频处理失败：无法提取音频")
            return
        audio_to_process = audio_path
        # 使用原视频文件名作为前缀
        if not filename_prefix:
            filename_prefix = os.path.splitext(os.path.basename(audio_input))[0]
    
    result = transcribe_audio(audio_to_process, log_cb, is_local_file)
    if not result:
        log(f"第 {file_index} 个文件处理失败")
        return

    utters = result.get("result", {}).get("utterances", [])
    if not utters:
        log(f"第 {file_index} 个文件未获取到分句信息")
        return
